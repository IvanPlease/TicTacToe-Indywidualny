...
public class SceneSetter {
    ...
    public Scene gameScene(Stage primaryStage, String nickname, String difficulty, int n) throws IOException {
        ...
        for(int s = 0;s<squares.length;s++){
            for(int p = 0;p<squares[s].length;p++){
                ...
                float paneWidth = 360/n;
                square.setPrefSize(paneWidth, paneWidth);
                switch (squares[s][p]){
                    case 0:
    ...
    public Scene askScene(Stage primaryStage) throws IOException {
        ...
        sizeText.setText(sizePrompt);
        sizeText.setFill(Color.web(textColor));

        size.setCenter(sizeText);

        TextField sizeTextField = new TextField();
        sizeTextField.setId(sizeTextId);
        sizeTextField.getStyleClass().add(textAskClass);
        ...
        nextBtn.setOnMouseClicked(event -> {
            ...
            int sizeIn = Integer.parseInt(sizeTextField.getText());
            String diff = (String) difficulty.getSelectedToggle().getUserData();
            if(nic != null && !nic.isEmpty() && sizeIn > 0 && sizeIn <= 5){
                try {
                    primaryStage.setScene(this.gameScene(primaryStage, nic, diff, sizeIn));
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
        ...
        insidePane.add(sizeTextField,0,3);
